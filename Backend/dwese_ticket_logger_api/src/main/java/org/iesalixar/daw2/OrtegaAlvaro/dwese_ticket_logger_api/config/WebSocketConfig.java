...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    /**
     * Interceptor para manejar la autenticación basada en JWT en los mensajes entrantes.
    ...
    public void configureClientInboundChannel(ChannelRegistration registration) {
        registration.interceptors(new ChannelInterceptor() {
            ...
            public Message<?> preSend(Message<?> message, MessageChannel channel) {
                ...
                // Solo nos interesa validar en el momento de la conexión (CONNECT)
                // O si deseas validar cada mensaje, puedes quitar esta condición del if command
                if (StompCommand.CONNECT.equals(accessor.getCommand())) {

                    String token = accessor.getFirstNativeHeader("Authorization");
                    logger.info("☁️ WebSocket intentando autenticar...");
                    ...
                    if (token != null && token.startsWith("Bearer ")) {
                        ...
                        if (jwtUtil.validateToken(token, username)) {
                            ...
                            // Asocia la autenticación a la sesión del WebSocket
